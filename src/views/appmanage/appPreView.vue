    ...
    <div class="previewapp-wrapper">
      ...
      <div :class="getContentClass()" v-if="this.appBaseData">
        ...
        <div class="preview-mobilewrapper" v-show="!isPhone">
          <img class="mobieImg" src='../../assets/img/ic_mobilphone.png'>
...
<script type="text/ecmascript-6">
  ...
  function getLocalAppInfoByShortUrl() {
        var res = {}
        res.data = {}
        res.success = true
        res.data.app = {}
        res.data.app.createAt = '2019-01-16T08:57:27.799Z'
        res.data.app.appName = 'Chrome 浏览器'
        res.data.app.installWithPwd = false
        res.data.app.icon = 'upload/5be0060eff2d9261ee8b387f/icon/Chrome_128px.png'
        res.data.app.shortUrl = 'winChrome'
        res.data.version = {}
        res.data.version.uploadAt = '2019-01-16T11:39:06.962Z'
        res.data.version.versionStr = '62.0.3202.94'
        res.data.version.downloadUrl = 'upload/5be0060eff2d9261ee8b387f/win/ChromeStandalone_62.0.3202.94_Setup.exe'
        res.data.version.size = '47081576'
        res.data.version.installUrl = ''
  ...
  export default {
    ...
    methods: {
      ...
      //   }, reject => {
      //     this.$message.info('here')
      //     this.$message.error('服务器错误')
      //   })
      // },
      getAppInfo(shortUrl) {
        var res = getLocalAppInfoByShortUrl()
        // console.log(res)
        if (res.data.version === null) {
            this.$message.error('未检测到版本信息')
            return
...
//            `itms-services://?action=download-manifest&url=${this.axios.defaults.baseURL}api/plist/${this.appBaseData._id}/${this.appVersionInfo._id}`
          if (this.appVersionInfo.installUrl === '') {
            this.$message.error('未检测到版本信息，暂不能安装!')
            return
          }
...
<style lang="scss">
  ...
  .previewapp-wrapper {
    ...
    width: 100%;
    height: 100%;
    background-image: url("../../assets/img/bg_picture.png");
